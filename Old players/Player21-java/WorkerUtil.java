...
public class WorkerUtil {
    ...
    //first rounds
    static HashMap<Integer, Integer> ExploreMap;
    static final int MAX_FACTORIES = 5;
    //todo maybe fer en funcio de lo gran que es el mapa, lo lluny que estem dels workers enemics, maxima karbonite en una casella etc.
    static final double KARBONITE_CONSTANT = 10;
    //ronda fins la qual els nous workers recullen karbo
    static int EXPLORE_ROUND;
    //distancia fins la qual explorarem
    static double KARBO_DISTANCE;
    ...
    static int getKarboniteAround(AuxMapLocation loc, int r) {
        try {
            if(!loc.isOnMap()) return 0;
            if(!Data.accessible[loc.x][loc.y]) return 0;
            int ans = 0;
            int workers = 0;
            for (int i = 0; i < Vision.Mx[r].length; ++i) {
                ...
                if (d * d > r) continue;
                ans += Data.karboMap[mLoc.x][mLoc.y]*d*(1/r);
            }
            return ans;
        } catch (Exception e) {
            System.out.println(e);
    ...
    static void fillKarboniteAround(){
        try{
            karboniteAround = new int[Data.W][Data.H];
            for(int i = 0; i < Data.W; ++i){
                for(int j = 0; j < Data.H; ++j){
                    int ka = 0;
                    ...
                    for(int d = 0; d < 9; ++d){
                        ...
                        ka += getKarboniteAround(newLoc, 4);
                    }
                }
            }
        } catch(Exception e){
    ...
    static void decreaseKarboniteAround(AuxMapLocation loc, int r){
        try {
            int ans = 0;
            int workers = 0;
            for (int i = 0; i < Vision.Mx[r].length; ++i) {
            ...
            karboniteAround[loc.x][loc.y] = Math.min(karboniteAround[loc.x][loc.y] - ans, 0);
        } catch (Exception e) {
            System.out.println(e);
        }
    }
    ...
    //calcula la karbo distances des d'on soc
    //static AuxUnit[] myUnits; //myUnits[i] retorna una unit random meva
    //    static AuxUnit[] enemies;
    static void getKarboConstants(){
        try {
            AuxUnit[] myUnits = Data.myUnits;
            AuxUnit[]enemyUnits = Data.enemies;
            double minDist = INF;
            for(int i = 0; i < myUnits.length; ++i){
                for(int j = 0; j < enemyUnits.length; ++j){
                    double d = myUnits[i].getMaplocation().distanceBFSTo(enemyUnits[j].getMaplocation());
                    System.out.println("comparing my unit: ");
                    System.out.print(myUnits[i].getMaplocation().x);
                    System.out.print(" ");
                    System.out.println( myUnits[i].getMaplocation().y);
                    System.out.println("with");
                    System.out.print(enemyUnits[j].getMaplocation().x);
                    System.out.print(" ");
                    System.out.println( enemyUnits[j].getMaplocation().y);
                    System.out.print("distance: ");
                    System.out.println(d);
                    if(d < minDist){
                        minDist = myUnits[i].getMaplocation().distanceBFSTo(enemyUnits[j].getMaplocation());
                    }
                }
            }
            System.out.print("min dist: ");
            System.out.println(minDist);
            KARBO_DISTANCE = minDist/2; //TODO potser partir algo mes gran per no anar juuust al mig
            EXPLORE_ROUND = (int) KARBO_DISTANCE;
        }catch(Exception e){
            System.out.println(e);
        }
