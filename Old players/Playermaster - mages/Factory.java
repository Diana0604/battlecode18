...
public class Factory {
    ...
    private UnitType chooseNextUnit(){
        try {
            ...
            if (Danger.knightSeen && typeCount.get(UnitType.Mage) < 2) return UnitType.Mage;
            if (Units.healingPower > 15 && typeCount.get(UnitType.Ranger)/(typeCount.get(UnitType.Mage)+1) > 10) return UnitType.Mage;

            AuxUnit[] enemies = Wrapper.senseUnits(unit.getMapLocation(), 18, false);
            for (AuxUnit enemy : enemies){
                if (enemy.getMapLocation().distanceBFSTo(unit.getMapLocation()) <= 3 && knights < 2 && rangers <= 2){
                    if (knights == 1) return UnitType.Mage;
