        ...
        while (!q.isEmpty()) {
            ...
            if (y == 0){
                for (int j = 0; j < UnitManager.enemyUnits.size(); ++j){
                    if (!visitedEnemyUnits[j] && dangEnemyUnits[j] && locEnemyUnits[j].distanceSquaredTo(locUnits[x]) <= 68){
                        q.add(encode(j, 1));
                ...
                for (int j = 0; j < UnitManager.units.size(); ++j){
                    if (!visitedUnits[j] && dangUnits[j] && locUnits[j].distanceSquaredTo(locEnemyUnits[x]) <= 68){
                        q.add(encode(j, 0));
