        ...
        while (!q.isEmpty()) {
            ...
            if (y == 0){
                for (int j = 0; j < Data.enemies.length; ++j){
                    if (!visitedEnemyUnits[j] && dangEnemyUnits[j] && locEnemyUnits[j].distanceSquaredTo(locUnits[x]) <= Wrapper.getAttackRange(Data.enemies[j].getType())){
                        q.add(encode(j, 1));
                ...
                for (int j = 0; j < Data.myUnits.length; ++j){
                    ...
                    if (!visitedUnits[j] && dangUnits[j] && locUnits[j].distanceSquaredTo(locEnemyUnits[x]) <= Wrapper.getAttackRange(Data.myUnits[j].getType())){
                        q.add(encode(j, 0));
