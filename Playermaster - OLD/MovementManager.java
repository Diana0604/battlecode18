...
public class MovementManager {
    ...
    public int move(AuxUnit unit, int priority){
        try {
            ...
            for (int i = 0; i < 8; ++i){
                AuxMapLocation newLoc = myLoc.add(i);
                if (newLoc.isPushable()) directions.add(i);
            }
            if (priority != FORCED) directions.add(8);


            directions.sort((a,b) -> bestIndex(a,b) == b ? 1 : bestIndex(b,a) == a ? -1 : raw_dist(a, NOTFORCED) < raw_dist(b, NOTFORCED) ? -1 : raw_dist(a, NOTFORCED) == raw_dist(b, NOTFORCED) ? 0 : 1);
            ...
            for (int i = 0; i < directions.size(); ++i) {
                ...
                if (u != null) {
                    ...
                    if (u.getType() != UnitType.Factory && u.getType() != UnitType.Rocket && u.canMove()) {
                        if (move(u, Math.max(priority, PLSMOVE)) != 8) {
                            doMovement(unit, dirBFS);
                            return dirBFS;
                        }
                    }
