...
public class Mage {
    ...
    void doAction(AuxUnit _unit){
        try {
            ...
            unit = _unit;
            if (Units.attack() && unit.target != null && !unit.exploretarget && unit.target.distanceSquaredTo(unit.getMapLocation()) < 90) {
                if (trySpecialMove()) return;
                if (trySpecialMove2()) return;
            }
            attack();
        } catch(Exception e) {
            e.printStackTrace();
    ...
    private boolean trySpecialMove2(){
        if (!unit.canAttack()) return false;
        attack();
        if (!unit.canMove()){
            if(!Overcharge.getOvercharged(unit)){
                return false;
            }
        }
        MageMovement m = new MageMovement(new AuxMapLocation(0,0), 8, true);
        if (m.getValue() < min_group){
            MovementManager.getInstance().move(unit, true);
        }

        if (!unit.canMove()) trySpecialMove2();
        return false;
    }
