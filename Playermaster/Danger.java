        ...
        try {
            ...
            try {
                ...
                if (Mapa.onEarth()) rockets = Rocket.rocketTakeoffs;
                    ...
                    rockets = new HashSet<>();
                    for (int i = 0; i < 3; ++i) {
                        HashSet<AuxMapLocation> rocketsAux = Rocket.rocketLandingsByRound.get(Utils.round + 1 + i);
                        if (rocketsAux != null) rockets.addAll(rocketsAux);
                    }
                }
                for (AuxMapLocation rocketLoc : rockets) {
                    for (int j = 0; j < 9; ++j) {
                        if (!Wrapper.canMove(unit, j) && j < 8) continue;
                        AuxMapLocation newLoc = myLoc.add(j);
                        if (rocketLoc.distanceSquaredTo(newLoc) <= 2) {
                            data.DPS[j] += 100;
                            data.DPSlong[j] += 100;
                        }
                    }
